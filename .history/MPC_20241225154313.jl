...
data_new.session_end_time_la = [DateTime(x, dateformat"mm/dd/yyyy HH:MM") for x in data_new.session_end_time_la]
data_new.charging_time_secs = [tryparse(Time, x, dateformat"HH:MM:SS") |> t -> isnothing(t) ? missing : Hour(t.hour)*3600 + Minute(t.minute)*60 + Second(t.second) for x in data_new.charging_time_secs]
