        ...
        # Real-time update at every time step: only arrived EVs can be charged and considered in the MPC
            ...
            if (method == "Persistence+KNN" || method == "Statistic+KNN") 
                ...
                elseif !isempty(arrived_sessions_today)
                    ...
                    elseif !isempty(arrived_sessions_forecast)
                        # NOTE: for persistence forecast ED could be super large that full power charging cannot meet the ED
                        # VERSION: change the ED accordingly
                        ED_candidate = noarrived_sessions_forecast.ED * sqrt(size(arrived_sessions_today, 1) / size(arrived_sessions_forecast, 1))
                        noarrived_sessions_forecast.ED = min.(ED_candidate, P_max * (ceil.(Int, noarrived_sessions_forecast.PD / T * N)) * delta_t)
                        # VERSION: replace with the historical session
                        if size(arrived_sessions_today, 1) > size(arrived_sessions_forecast, 1)
