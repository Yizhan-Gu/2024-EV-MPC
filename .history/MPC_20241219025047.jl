...
# MPC main function
function run_mpc(data_input::DataFrame, data_today::DataFrame, method::String, base::String)
    ...
    @showprogress for k in 1:N
        ...
        # Real-time update at every time step: only arrived EVs can be charged and considered in the MPC
            ...
            if method == "Noforecast"
                ...
                elseif isempty(arrived_sessions_forecast)
                    data_forecast_update = copy(data_input[1:2, :])
        ...
        elseif method == "Charger"
            ...
            # Constraints
            for j in 1:N_charger
                # Extract the AT and DT for the j-th charger
                AT_values = combined_data_forecast_update.AT[j]
                DT_values = combined_data_forecast_update.DT[j]
            
                # Ensure AT_values and DT_values are vectors (adjust if they are not)
                for i in 1:length(AT_values)
                    at = AT_values[i]
                    dt = DT_values[i]
            
                    # Loop over time steps (k)
                    for k in 1:N
                        # Add constraint P[k, j] == 0 for invalid k ranges
                        if k < at || k > dt  # Adjust based on your invalid ranges logic
                            @constraint(model, P[k, j] == 0)
                        end
                    end
                end
            end
