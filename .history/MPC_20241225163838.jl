...
CP_data_clean = filter(row -> (Dates.value(row.session_end_time_la) - Dates.value(row.session_start_time_la)) / 60 >= 10, CP_data_clean)
...
CP_data_clean_train = filter(row -> Dates.year(row.session_start_time_la) < 2023, CP_data_clean)
CP_data_clean_test = filter(row -> Dates.year(row.session_start_time_la) == 2023 && Dates.month(row.session_start_time_la) <= 9, CP_data_clean)
