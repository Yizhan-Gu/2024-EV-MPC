...
function run_mpc(data_forecast::DataFrame, data_today::DataFrame, method::String, base::String)
    ...
    @showprogress for k in 1:N
        ...
        elseif base == "Charger" # VERSION: base selection
            ...
            # Find rows in unique_stations_ports not in chargers_update
            missing_rows_mask = .!(all(row -> any(r -> r == row, eachrow(chargers_update)), eachrow(unique_stations_ports)))
            missing_rows = unique_stations_ports[missing_rows_mask, :]

            # Assign default values
            missing_rows.AT = fill([8.0], nrow(missing_rows))  # 8:00 AM as float
            missing_rows.DT = fill([8.0], nrow(missing_rows))  # 8:00 AM as float
            missing_rows.ED = fill([0.0], nrow(missing_rows))  # Energy Demand as 0
