...
function run_mpc_ev(data_input::DataFrame, data_today::DataFrame, method::String)
    ...
    @showprogress for k in 1:N
        ...
        elseif (method == "Noforecast")
            data_forecast_update = DataFrame()
            update_time = (k - 1) * delta_t

            arrived_sessions_today = filter(row -> row.AT <= update_time, data_input)
            data_forecast_update = copy(arrived_sessions_today)
        ...
        elseif (method == "Persistence+KNN" || method == "Statistic+KNN")
            ...
            elseif !isempty(arrived_sessions_today)
                ...
                if isempty(arrived_sessions_step) && !isempty(arrived_sessions_step_forecast)
                    # Set ED to 0 for rows in data_forecast_update where AT matches any AT in arrived_sessions_step_forecast
                    redundant_sessions = filter(row -> !(row.AT in arrived_sessions_step_forecast.AT), noarrived_sessions_forecast)
                    redundant_sessions.ED .= 0
                end
        ...
        # FIXME: gradient descent step length
        ...
        for i in 1:N_ev
            ...
            elseif AT_idx[i] <= k && DT_idx[i] > k
                ...
                @constraint(model, [t=DT_idx[i]:N], P[t, i] == 0)
            elseif DT_idx[i] <= k
                @constraint(model, [t=k:N], P[t, i] == 0)
            elseif AT_idx[i] > k
...
# Energy plot
# TODO: plot energy by time, should be real the biggest with the rest approximating it
df_energy_real = 
df_energy_V0G = 
df_energy_perfect = DataFrame(time = [DateTime(minimum(days)) + Minute(15 * (i - 1)) for i in 1:N * length(days)], energy = vcat(values(E_mpc_perfect)...))
df_energy_noforecast = DataFrame(time = [DateTime(minimum(days)) + Minute(15 * (i - 1)) for i in 1:N * length(days)], energy = vcat(values(E_mpc_noforecast)...))
df_energy_persistence_knn = DataFrame(time = [DateTime(minimum(days)) + Minute(15 * (i - 1)) for i in 1:N * length(days)], energy = vcat(values(E_mpc_persistence_knn)...))
df_energy_statistic_knn = DataFrame(time = [DateTime(minimum(days)) + Minute(15 * (i - 1)) for i in 1:N * length(days)], energy = vcat(values(E_mpc_statistic_knn)...))

p_energy_combined = plot(df_energy_V0G.time, df_energy_V0G.energy,
    label="V0G",  # Add cost to label
    xlabel="Timestamp",
    ylabel="Energy (kWh)",
    size=(1400, 600),
...
plot!(df_energy_perfect.time, df_energy_perfect.energy, label="Perfect Forecast")
plot!(df_energy_noforecast.time, df_energy_noforecast.energy, label="No Forecast")
plot!(df_energy_persistence_knn.time, df_energy_persistence_knn.energy, label="Persistence+KNN Forecast")
plot!(df_energy_statistic_knn.time, df_energy_statistic_knn.energy, label="Statistic+KNN Forecast")
savefig(p_energy_combined, "Energy_All.png")
