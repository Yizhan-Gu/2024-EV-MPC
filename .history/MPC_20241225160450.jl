...
####################################################
# Data preprocessing
CP_data = CSV.read("CP_UCSD_raw_Jul16_Sep24.csv", DataFrame)
print(first(CP_data))

CP_data_clean = select(CP_data,
"User ID" => :driver_id,
"Start Date" => :session_start_time_la,
"End Date" => :session_end_time_la,
"Energy (kWh)" => :total_energy_dispensed,
"Charging Time (hh:mm:ss)" => :charging_time_secs,
"Station Name" => :station_name,
"Port Number" => :port)

CP_data_clean = dropmissing(CP_data_clean)
CP_data_clean.session_start_time_la = [DateTime(x, dateformat"mm/dd/yyyy HH:MM") for x in CP_data_clean.session_start_time_la]
CP_data_clean.session_end_time_la = [DateTime(x, dateformat"mm/dd/yyyy HH:MM") for x in CP_data_clean.session_end_time_la]
CP_data_clean.charging_time_secs = [
    tryparse(Time, x, dateformat"HH:MM:SS") for x in CP_data_clean.charging_time_secs
]
CP_data_clean.charging_end_time_la = [
    (session_start + (t.hour * 3600 + t.minute * 60 + t.second)) for (session_start, t) in zip(CP_data_clean.session_start_time_la, CP_data_clean.charging_time_secs)
]
...
function run_mpc(data_forecast::DataFrame, data_today::DataFrame, method::String, base::String)
    ...
    @showprogress for k in 1:N
            ...
            if (method == "Persistence+KNN" || method == "Statistic+KNN") 
                ...
                elseif !isempty(arrived_sessions_today)
                    ...
                    elseif !isempty(arrived_sessions_forecast)
                        ...
                        =#
                        # TODO: ML method to predict the ED
                    end
        ...
        if k == 1
            @expression(model, E_dispensed, 0)
        else
            if base == "EV" 
        ...
        p0 = plot(ED_vector, label="ED", title="Energy Demand Check")
