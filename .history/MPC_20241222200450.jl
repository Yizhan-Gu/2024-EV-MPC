...
function run_mpc(data_forecast::DataFrame, data_today::DataFrame, method::String, base::String)
    ...
    @showprogress for k in 1:N
        ...
        elseif base == "Charger"
            ...
            if !(all(row -> in(row, eachrow(unique_stations_ports)), eachrow(chargers_update)))
                error("New station not in history records")
            end
            missing_rows = filter(row -> !in(row, eachrow(unique_stations_ports[:, [:station_name, :port]])), eachrow(combined_data_forecast_update))
