        ...
        elseif method == "Charger"
            ...
            for i in 1:N_charger
                ...
                for j in eachindex(AT_values)
                    ...
                    # TODO: modify the new constraints for charger
                    if at_idx <= k && dt_idx >= k
                        @constraint(model, E[dt_idx, i] == ED[i]) # Strict
                        if DT_idx[i] <= N - 1
                            @constraint(model, [t=DT_idx[i]+1:N], P[t, i] == 0)
                        end
                    else
                ...
                @constraint(model, [t=k:N], 0 <= P[t, i] <= P_max)
                @constraint(model, P[t, i] == 0 for t in valid_vacant_idx)
