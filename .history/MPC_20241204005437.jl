...
function run_mpc_ev(data_input::DataFrame, data_today::DataFrame, method::String)
    ...
    @showprogress for k in 1:N
        ...
        # Real-time update at every time step: only arrived EVs can be charged and considered in the MPC
            ...
            arrived_sessions_redundant_forecast = filter(row -> arrived_sessions_today.AT[size(arrived_sessions_today, 1)] < row.AT <= update_time, data_input)
            ...
            elseif (method == "Persistence+KNN" || method == "Statistic+KNN")
                ...
                elseif !isempty(arrived_sessions_today)
                    ...
                    if isempty(arrived_sessions_step)
                        arrived_sessions_redundant_forecast.AT .= min.(arrived_sessions_redundant_forecast.AT .+ delta_t * k, 24)
                        arrived_sessions_redundant_forecast.DT .= min.(arrived_sessions_redundant_forecast.DT .+ delta_t * k, 24)
                    end
