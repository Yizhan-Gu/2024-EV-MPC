...
CP_data_clean = filter(row -> 
    ...
    (minute(row.session_end_time_la) - minute(row.session_start_time_la)) > 25, 
    CP_data_clean)
...
data_test_candidate.overMaxPower = data_test_candidate.ED .> P_max * delta_t * (data_test_candidate.DT_idx - data_test_candidate.AT_idx)
